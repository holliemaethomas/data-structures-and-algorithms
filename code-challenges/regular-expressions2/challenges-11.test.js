...
/* -----------------------note I got most of my regex from stack-overflow-------------------------------------------------------------------------
CHALLENGE 1
...
const validatePin = (pin) => {
  if (/^[0-9]{4}$/g.test(pin)) {return true}
  return false;
};
...
const validateEmail = (email) => {
  if (email.match(/^\w+@\w+\.(com|net|org)$|^\w+\.\w+@\w+\.(com|net|org)/g)) { return true }
  return false;
}
...
const validatePhoneNumber = (phoneNumber) => {
  if ((phoneNumber.match(/^(([(][0-9]{3}[)])|([0-9]{3}))(|-|[ ])([0-9]{3})(|-|[ ])([0-9]{4}$)/g))) { return true };
  return false;
};
