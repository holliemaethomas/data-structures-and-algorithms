...
const hasChildrenValues = (arr, character) => {
  let children = false;
  arr.forEach(person => {
    let dependantCheck = Object.values(person);
    if (dependantCheck[0] === character) {
      if (dependantCheck[2].length > 0) { children = true }
    }
  });
  return children;
...
const hasChildrenEntries = (arr, character) => {
  let children = false;
  arr.forEach(person => {
    let dependantCheck = Object.entries(person)
    if (dependantCheck[0][1] === character) {
      if (dependantCheck[2][1].length > 0) { children = true }
    }
  })
  return children;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(people => {
    total++
    if (people.spouse){total++}
    total += people.children.length;
  })
  return total;
};
