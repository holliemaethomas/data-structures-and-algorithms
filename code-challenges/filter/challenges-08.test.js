...
const oddValues = (arr) => {
  const returns = arr.filter(val => val % 2);
  return returns;
};
...
const filterStringsWithVowels = (arr) => {
  const returns = arr.filter(value => {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const returns = arr.filter(value => {
    if (minBaseStat < value.baseStat) { return true }
    return false;
  });
  return returns;
};
...
const getStatName = (arr, minBaseStat) => {
  const returnArr = [];
  arr.filter(value => {
    if (minBaseStat < value.baseStat) { returnArr.push(value.stat.name) }
    return false;
  })
  return returnArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  const returns = arr.filter(value => {
    if (!Object.keys(value).includes('children')) { return true }
    return false;
  });
  return returns;
};

/* nich paro worked with me on these over break-------------------------------------------------------------------------------------
