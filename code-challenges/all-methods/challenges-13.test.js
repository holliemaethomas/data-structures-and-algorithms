...
/* -------------------------travis helped me with this one-----------------------------------------------------------------------
CHALLENGE 2
...
/* ---------------------------------------------------------------------------------nich psro helped with this lab---------------
CHALLENGE 4
...
const findEvery = (arr, target) => {
  ...
  arr.forEach(exsists => {
    if (!exsists.includes(target)) { containsTarget = false }
  })
...
const unenrollBrook = (arr) => {
  ...
  arr.forEach(roster => {
    let inside = [];
    roster.map(person => {
      if (!person.includes('Brook')) { inside.push(person) }
    })
    res.push(inside)
    inside = [];
  })
