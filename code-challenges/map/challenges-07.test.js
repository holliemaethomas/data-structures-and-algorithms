...
const forEachTwoToThe = (arr) => {
  const returnArr = [];
  arr.forEach(value => returnArr.push(2 ** value));
  return returnArr;
}
...
const mapTwoToThe = (arr) => {
  const twoMap = arr.map(value => 2 ** value);
  return twoMap;
};
...
const charCode = (arr) => {
  const charMap = arr.map((value, index) => arr[index].charCodeAt());
  return charMap;
};
...
const extractStats = (arr) => {
  ...
  Object.values(arr).map(value => {
    const basePower = new Object();
    basePower.name = value.stat.name;
    basePower.total = value.effort + value.baseStat;
    returnArr.push(basePower);
  })
  return returnArr
};

/* got help on thi one from Nich Paro------------------------------------------------------------------------------------------------
TESTS
