...
const totalSum = (input) => {
  let addItUp = 0;
  input.map(val => val.map(val2 => addItUp += val2));
  return addItUp
...
/* --------received help on this challenge from nich paro-----------------------------------------------------------------------
CHALLENGE 4
...
let findShortest = (data) => {
  ...
  const results = [];
  data.map(person =>
    results.push(parseInt(person.height)))
  ...
  data.map(person => {
    if (person.height === shortest) { answer = person.name }
  });
  return answer;
};
